...
## v0.3.4 build fixes
- Defines `NT_PRSTATUS` locally for Android NDK environments where the constant is not exposed by the available headers.
- Passes `lib.c_str()` to `remote_dlopen(const char*)`.
